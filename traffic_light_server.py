...
# A lock to ensure that only one thread can modify the state variables at a time.
state_lock = threading.Lock()

# "target" variables are goals set by the web server based on user clicks.
target_mode = "auto"
target_manual_color = "off"

# "current" variables represent the actual, real-time state of the system.
current_mode = "auto"
current_color = "unknown"
last_state_change_time = 0

# Variables to hold data from background threads.
s_bahn_minutes_away = -1 # -1 means unknown/no train
weather_status = {}      # e.g., {'temp': 20, 'condition': 'Clear'}
...
def set_light_state(color_to_set):
    ...
    if current_color == color_to_set:
        return # Optimization: Don't change hardware if state is already correct.
...
def get_next_train_minutes(eva_number, client_id, client_secret):
    """Fetches and parses train data from the DB API."""
...
def traffic_light_controller():
    ...
    global current_mode, last_state_change_time
    
    # Mode-specific state variables that need to persist between loops
    mode_state = {
    ...
    while True:
        ...
        with state_lock:
            ...
            # This is a dispatch table that calls the correct function for the current mode.
            # It's a cleaner way to handle the different modes than a long if/elif chain.
            mode_handlers = {
            ...
            if handler:
                loop_sleep = handler(elapsed, mode_state) or 0.1

        sleep(loop_sleep)

def handle_auto_mode(elapsed, state):
    """Logic for the German traffic light sequence."""
    global last_state_change_time
    if current_color == 'green' and elapsed > 20: set_light_state('yellow'); state['next_auto_state'] = 'red'; last_state_change_time = time()
    elif current_color == 'yellow' and elapsed > 3: set_light_state(state['next_auto_state']); last_state_change_time = time()
    elif current_color == 'red' and elapsed > 20: set_light_state('red_and_yellow'); state['next_auto_state'] = 'green'; last_state_change_time = time()
    elif current_color == 'red_and_yellow' and elapsed > 2: set_light_state(state['next_auto_state']); last_state_change_time = time()

def handle_party_mode(elapsed, state):
    """Logic for Party mode."""
    set_light_state(random.choice(['red', 'yellow', 'green', 'off']))
    return 0.08 # Faster loop speed

def handle_emergency_mode(elapsed, state):
    """Logic for Emergency mode."""
    set_light_state('yellow' if current_color != 'yellow' else 'off')
    return 0.5

def handle_sos_mode(elapsed, state):
    """Logic for SOS mode."""
    global last_state_change_time
    current_step = state['sos_pattern'][state['sos_index']]
    if elapsed > current_step['duration']:
        state['sos_index'] = (state['sos_index'] + 1) % len(state['sos_pattern'])
        set_light_state(state['sos_pattern'][state['sos_index']]['state'])
        last_state_change_time = time()

def handle_s_bahn_mode(elapsed, state):
    """Logic for S-Bahn countdown mode."""
...
def handle_biergarten_mode(elapsed, state):
    """Logic for Biergarten recommendation mode."""
...
class StatusHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        """Handle GET requests for status, actions, and the main page."""
...
def get_html_content():
    """Returns the full HTML content for the web interface."""
    return f"""
    <!DOCTYPE html>
    <html lang="en">
    <head>
        ...
        <style>
            :root {{
                --bg-color: #1a1d23; --body-bg: #111317; --text-color: #e0e0e0;
                --text-muted: #888; --accent-color: #007bff; --shadow-color: rgba(0, 0, 0, 0.5);
            }}
            html, body {{
                height: 100%; margin: 0; padding: 0; background-color: var(--body-bg);
                font-family: 'Inter', -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif;
                color: var(--text-color); -webkit-tap-highlight-color: transparent;
                display: flex; justify-content: center; align-items: center;
            }}
            .container {{
                width: 100%; max-width: 380px; padding: 20px; box-sizing: border-box;
                display: flex; flex-direction: column; align-items: center; gap: 25px;
            }}
            .traffic-light-body {{
                background-color: var(--bg-color); border-radius: 24px; padding: 20px;
                display: flex; flex-direction: column; gap: 15px; border: 1px solid #333;
                box-shadow: 0 10px 30px var(--shadow-color);
            }}
            .light {{
                width: 90px; height: 90px; border-radius: 50%; background-color: #333;
                opacity: 0.5; transition: all 0.15s ease-in-out; cursor: pointer;
            ...
            #modeText {{ font-size: 1.5em; font-weight: 600; margin-top: 0; margin-bottom: 8px; }}
            .info-text {{ height: 22px; font-size: 1em; font-style: italic; color: var(--text-muted); margin-bottom: 20px; }}
            .mode-buttons {{ display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 10px; width: 100%; }}
            .mode-buttons a {{
                background-color: #333; color: var(--text-color); padding: 12px 10px;
                border-radius: 12px; font-size: 1em; font-weight: 600; text-decoration: none;
                transition: background-color 0.2s, transform 0.1s;
            }}
            .mode-buttons a:active {{ transform: scale(0.95); }}
            .mode-buttons a.active {{ background-color: var(--accent-color); color: white; }}
        </style>
    </head>
    <body>
        <div class="container">
        ...
        <script>
            ...
            function stopLocalAnimation() {{
                if (localAnimationId) {{ clearInterval(localAnimationId); clearTimeout(localAnimationId); localAnimationId = null; }}
            }}
